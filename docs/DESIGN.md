...
| Connection setup | **Both**: env vars for bootstrap/docker-compose, plus in-app settings pages for changes afterward. The separate first-run wizard was dropped in Phase 9 — the individual pages cover it |
| License | MIT |
| Name | **Franchisarr** |
...
| Cross-instance "already have it" (e.g. 4K vs. 1080p Radarr) | **Instances independent by default**, with a **settings toggle** to treat "in any instance" as not-missing, for setups that split by content type rather than quality tier (note: the developer turns out to run a single Sonarr, handling anime via quality profiles, so this toggle's multi-instance behaviour is covered by tests rather than by their hardware) |
...
18. **Update check without leaking install details.** *(Phase 9: points at the Forgejo releases *list* rather than `/releases/latest`, which 404s on a repository with no releases yet and is then indistinguishable from a broken URL. Both Forgejo and GitHub return that list newest-first with the same fields, so the GitHub migration is a hostname change. Overridable per install via `update_releases_url`; drafts and pre-releases are never announced.)* The release-check call should hit the repo's public release API (no telemetry about the user's own library/config) and fail silently/log-only if the repo is unreachable (private Forgejo instance, offline host, etc.) — never block app startup or normal use on this check succeeding.
...
22. **Monitor-mode mapping to Sonarr's actual API.** Sonarr's `addOptions.monitor` field has specific accepted values that don't map 1:1 to a friendly "All/Future Only/First Season" UI — worth confirming Sonarr's current v3 API enum values during Phase 6/7 rather than assuming the UI labels translate directly. *(Phase 7 prep, resolved against a real Sonarr 4.0.19 and its source): `MonitorTypes` is `Unknown, All, Future, Missing, Existing, FirstSeason, LastSeason, [obsolete] LatestSeason, Pilot, Recent, MonitorSpecials, UnmonitorSpecials, None, Skip`, serialised camelCase. The three friendly labels do map — All→`all`, Future Only→`future`, First Season→`firstSeason` — but not by name: our stored values are `all|future_only|first_season`, so a translation layer is required and the labels must not be sent verbatim. The instance exposes no OpenAPI document, so this came from Sonarr's own source rather than the running server.)
...
27. **Spin-off heuristics need precision, not recall.** *(found in Phase 6, resolved)* The title-prefix heuristic first allowed any whitespace after the source name, which on a real 656-show library produced 308 candidates that were overwhelmingly unrelated shows sharing a first word — "Angel" matched "Angel Beats!", "Angel Street" and "Angel City". Because this tier's entire cost is human attention, a false positive is more expensive than a miss. Requiring a colon or dash separator, plus dropping TMDb's per-season entries, cut it to 82 across 49 shows with most of them genuine. Spin-offs that don't carry the parent's name (Chicago Fire / Chicago P.D.) are unfindable this way by construction, and the UI says so rather than letting an empty result read as "no spin-offs exist".
28. **An SSO proxy in front of an *arr app looks exactly like a bad API key.** *(found in Phase 8, resolved)* Radarr and Sonarr answer an auth failure with JSON; a forward-auth proxy — Authelia, Authentik, Cloudflare Access, oauth2-proxy — answers 401 with an HTML login page or a redirect to one. Reporting that as "your API key was rejected" sends the user to check a credential that was never the problem, and this audience puts such proxies in front of their *arr apps constantly. Both clients now inspect the response body and, when it looks like a login page, say so and suggest either an internal URL or a `/api` bypass rule. Found because this developer's own instances are behind one.
...
5. **Phase 4 — Radarr integration + multi-instance — ✅ COMPLETE (validated against a real Radarr 6.3; web UI moved to Phase 5 so screens are built after theming):** Radarr client (incl. live profile/root-folder fetch with an unreachable-instance failure state), instance CRUD + setup wizard step, cross-instance dedup toggle, "add missing movie" (monitored + search-on-add) with instance dropdown, wired into web UI, each add recorded to `activity_log`.
6. **Phase 5 — Web UI polish for movies — ✅ COMPLETE:** collections-with-gaps list, missing/existing view (with upcoming/unreleased films kept visually separate per technical challenge #26), per-user dismiss action, per-collection "not part of my collection" exclude action, plus **theme.park theme support** — the optional stylesheet-URL setting and the mapping from theme.park's custom properties onto Pico's. Themeability lands here rather than with the rest of the settings work in Phase 9 so the first real screens are built against a themeable variable set, instead of being audited for hardcoded colours afterwards.
7. **Phase 6 — TV spin-offs — ✅ COMPLETE:** `spinoff_mappings` table starts empty, TV matcher, diff logic, CLI output, "add a spin-off mapping" UI action (always writes `source=local`).
...
- ~~Exact webhook payload shape for each `webhook_format`~~ — **resolved Phase 8**: generic is an untruncated JSON body (a machine consumer wants the whole list and there is no third-party cap); Discord uses one embed with a field per category; Slack sets `text` as well as `blocks`, since `text` is what appears in the notification popup. Discord and Slack truncate to 15 items with an "…and N more" line — a first scan found 227 missing films and both services reject an oversized payload outright.
...
- ~~Whether config export should offer a "redact secrets" option~~ — **resolved Phase 9**: both are offered. The full export is what restores an install; the redacted one is what gets pasted into a forum thread, which is exactly the moment people leak their keys. Importing a redacted file skips those instances rather than creating ones that cannot authenticate.