...
1. **Plex → TMDb ID matching.** Plex stores external IDs in `guid`/`Guid` fields (format differs: new Plex agent vs. legacy). Need a robust resolver with a title+year fuzzy-match fallback against TMDb search.
2. **Plex OAuth flow.** Uses plex.tv's PIN-based auth (`https://plex.tv/api/v2/pins`), then verifying the returned auth token actually has access to the specific Plex server this instance is configured for (via `https://plex.tv/api/v2/shared_servers` or `/api/resources`) — otherwise anyone with a Plex account could log in.
...
6. **Phase 5 — Web UI polish for movies:** collections-with-gaps list, missing/existing view, per-user dismiss action, per-collection "not part of my collection" exclude action.
...
- Exact Plex OAuth implementation detail (plex.tv PIN flow vs. a registered Plex "app" with client ID) — small decision, deferred to Phase 2 since it's an implementation detail, not a product decision.
...
- What email/contact address `SECURITY.md` should list before the GitHub migration (GitHub's private security advisories aren't available while the repo is on Forgejo) — needs a real inbox you're willing to monitor, not a placeholder.
